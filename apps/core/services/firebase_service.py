...
class FirebaseService:
    ...
    async def get_user_locations(self, user_id: str) -> List[Dict]:
        """Get user locations with full location data"""
        try:
            @sync_to_async
            def fetch_locations():
                user_locations = self.db.child('user_locations').child(user_id).get()
                if not user_locations:
                    return []

                locations = []
                for ul_id, ul_data in user_locations.items():
                    if not isinstance(ul_data, dict):
                        continue

                    # Get the associated location data
                    ...
                    # Combine location and user location data
                    combined_data = {
                        **location_data,
                        'user_location': {
                            'id': ul_id,
